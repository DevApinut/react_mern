...
const Contact = (prop) => {
    ...
    const Updatecontact = (e) => {
        e.preventDefault()
        axios.post(`${process.env.REACT_APP_API}/Updatecontact`, { nameThai: substation, Feeder: sublisttest, ...state, index_array: state.index_new_data }, {
            headers: {
    ...
    const fetch = (nameThai, index = 0) => {
        ...
        axios.post(`${process.env.REACT_APP_API}/fetchdatatest`, { nameThai }, {
            ...
            .then(res => {
                // console.log(prop.index_feeder)       
                dispatch({ type: 'Update_contact', payload: { ...res.data.result.switchgear_CB[index][prop.index_feeder].Contact } })
