...
const Counter = (prop) => {
    ...
    const fetch = (nameThai, index = 0) => {
        ...
        axios.post(`${process.env.REACT_APP_API}/fetchdatatest`, { nameThai }, {
            ...
            .then(res => {
                setfetchdata(res.data.result.Switch_yard[index])
    ...
    const Add_counter = async (e) => {
        ...
        axios.post(`${process.env.REACT_APP_API}/Updatecounter_Switchtyard `, { nameThai: substation, data: Counter, index_array: Number(index_new_data) }, {
            headers: {
                authorization: `Bearer ${getToken()}`
            }
        })
        ...
        <div>
            <form onSubmit={Add_counter}>
                ...
                <div className="d-flex justify-content-center">
                    <div>
                        {fetchdata.map(data => (
                            <div className="input-group mb-3" key={uuidv4()}>
                        ...
                        {fetchdata.map(data => {
                            return (data.Counter !== undefined) ?
                                <div className="input-group mb-3">
                                    <input type="text" className="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" defaultValue={data.Counter} key={uuidv4()} />
                                </div>
                                :
                                <div className="input-group mb-3">
                                    <input type="text" className="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" defaultValue="" key={uuidv4()} />
                                </div>
                        })}
                    </div>
                </div>
