...
const Insulation = (prop) => {
    ...
    const fetch = (nameThai, index = 0) => {
        ...
        axios.post(`${process.env.REACT_APP_API}/fetchdatatest`, { nameThai }, {
            ...
            .then((res) => {
                dispatch({ type: 'Update_insulation', payload: { ...res.data.result.Switch_yard[index][prop.index_feeder].Insulation } })
    ...
    const Add_insulation = (e) => {
        e.preventDefault();        
        axios.post(`${process.env.REACT_APP_API}/Updateinsulation_Switchtyard`, { nameThai: substation, Feeder: sublisttest, ...state, index_array: state.index_new_data }, {
            headers: {
                authorization: `Bearer ${getToken()}`
            }
        })
